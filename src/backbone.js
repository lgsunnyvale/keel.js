...
(function(){
  ...
  // Override Backbone.ajax to queue all non-GET requests to prevent lost updates.
  ...
  Backbone.ajax.requestNext = function() {
    ...
    if ((next = this.requests.shift())) {
      return this.request.apply(this, next); //change to apply here
    } else {
      return this.pending = false;
    }
  };

  Backbone.ajax.request = function(_options, proxy, trigger) {
    var ATTEMPTS, complete, error, options, params, success,
    _this = this;
    if (trigger === null) {
      trigger = true;
    }
    ...
    params = {
      ...
      success: function() {
        ...
        return proxy.resolveWith(this, arguments);
      },
      error: function(xhr, status, err) {
        if (status === 'timeout' && ATTEMPTS < MAX_ATTEMPTS) {
          ...
          return proxy.rejectWith(this, arguments);
        }
      }
    };
    ATTEMPTS = 1;
  ...
  Backbone.ajax.queue = function(options) {
    ...
    if (type === 'GET') {
      ...
      this.requests.push([options, proxy]);
    } else {
      this.pending = true;
      this.request(options, proxy);
    }
    return proxy;
  };
